...
--Description: This file includes the schema for the Blue Spoke database.
CREATE SCHEMA IF NOT EXISTS bike_shop;
SET search_path TO bike_shop,
    public;
...
) THEN CREATE TYPE appointment_status AS ENUM (
    'scheduled',
    'confirmed',
    'in_service',
    'done',
...
CREATE TABLE IF NOT EXISTS inventory_items (
    ...
    name TEXT NOT NULL,
    brand_id BIGINT REFERENCES brands(id) ON DELETE
    SET NULL,
...
CREATE TRIGGER inventory_items_set_updated_at BEFORE
UPDATE ON inventory_items FOR EACH ROW EXECUTE FUNCTION set_updated_at();
...
CREATE TABLE IF NOT EXISTS inventory_serials (
    ...
    location_id BIGINT REFERENCES locations(id) ON DELETE
    SET NULL,
...
CREATE TABLE IF NOT EXISTS work_orders (
    ...
    bike_id BIGINT REFERENCES customer_bikes(id) ON DELETE
    SET NULL,
        opened_by BIGINT REFERENCES users(id) ON DELETE
    SET NULL,
        assigned_to BIGINT REFERENCES users(id) ON DELETE
    SET NULL,
...
SELECT wo.id AS work_order_id,
    ...
    (
        COALESCE(SUM(sl.price * sl.quantity), 0) + COALESCE(SUM(pl.unit_price * pl.quantity), 0)
...
CREATE TABLE IF NOT EXISTS time_clock_entries (
    ...
    duration_minutes INT GENERATED ALWAYS AS (
        CASE
            WHEN clock_out IS NOT NULL THEN ROUND(
                EXTRACT(
                    EPOCH
                    FROM (clock_out - clock_in)
                ) / 60.0
            )::INT
        END
    ) STORED,
...
CREATE TABLE IF NOT EXISTS warranties (
    ...
    brand_id BIGINT REFERENCES brands(id) ON DELETE
    SET NULL,
        item_id BIGINT REFERENCES inventory_items(id) ON DELETE
    SET NULL,
...
CREATE TABLE IF NOT EXISTS customer_warranties (
    ...
    bike_id BIGINT REFERENCES customer_bikes(id) ON DELETE
    SET NULL,
        inventory_item_id BIGINT REFERENCES inventory_items(id) ON DELETE
    SET NULL,
        serial_number TEXT,
        ...
        end_date DATE GENERATED ALWAYS AS (
            start_date + make_interval(months => duration_months)
        ) STORED,
        notes TEXT
);
...
SELECT u.full_name AS mechanic,
    ...
    (
        EXTRACT(
            EPOCH
            FROM (a.end_at - a.start_at)
        ) / 60
    )::INT AS minutes_booked,
    a.status,
...
CREATE OR REPLACE FUNCTION bike_shop.wo_part_ins_consume() RETURNS trigger LANGUAGE plpgsql AS $$
DECLARE loc BIGINT;
BEGIN
SELECT location_id INTO loc
FROM bike_shop.work_orders
WHERE id = NEW.work_order_id;
INSERT INTO bike_shop.inventory_transactions(
        item_id,
        location_id,
        txn_type,
        quantity,
...
RETURN NEW;
END $$;
CREATE OR REPLACE FUNCTION bike_shop.wo_part_upd_adjust() RETURNS trigger LANGUAGE plpgsql AS $$
DECLARE loc BIGINT;
delta INT;
BEGIN
SELECT location_id INTO loc
...
RETURN NEW;
END $$;
CREATE OR REPLACE FUNCTION bike_shop.wo_part_del_revert() RETURNS trigger LANGUAGE plpgsql AS $$
DECLARE loc BIGINT;
BEGIN
SELECT location_id INTO loc
FROM bike_shop.work_orders
...
AFTER
INSERT ON work_order_part_lines FOR EACH ROW EXECUTE FUNCTION bike_shop.wo_part_ins_consume();
CREATE TRIGGER trg_wo_part_upd_adjust
AFTER
UPDATE OF quantity,
    item_id,
    work_order_id ON work_order_part_lines FOR EACH ROW EXECUTE FUNCTION bike_shop.wo_part_upd_adjust();
CREATE TRIGGER trg_wo_part_del_revert
AFTER DELETE ON work_order_part_lines FOR EACH ROW EXECUTE FUNCTION bike_shop.wo_part_del_revert();
-- Business hours
CREATE TABLE IF NOT EXISTS business_hours (
    id BIGSERIAL PRIMARY KEY,
    location_id BIGINT NOT NULL REFERENCES locations(id) ON DELETE CASCADE,
    dow SMALLINT NOT NULL CHECK (
        dow BETWEEN 0 AND 6
    ),
...
CREATE OR REPLACE FUNCTION bike_shop.is_slot_allowed(
        _loc BIGINT,
        _start TIMESTAMPTZ,
        _end TIMESTAMPTZ
    ) RETURNS BOOLEAN LANGUAGE sql STABLE AS $$ WITH hours AS (
        SELECT open_time,
            close_time
    ...
    clash AS (
        SELECT 1
        FROM bike_shop.closures c
        WHERE c.location_id = _loc
            AND tstzrange(c.starts_at, c.ends_at, '[)') && tstzrange(_start, _end, '[)')
        LIMIT 1
    )
SELECT EXISTS(
        SELECT 1
...
$$;
CREATE OR REPLACE FUNCTION bike_shop.validate_appointment() RETURNS trigger LANGUAGE plpgsql AS $$ BEGIN IF NEW.location_id IS NULL THEN NEW.location_id := (
        SELECT location_id
        FROM bike_shop.work_orders
        WHERE id = NEW.work_order_id
    );
END IF;
IF NOT bike_shop.is_slot_allowed(NEW.location_id, NEW.start_at, NEW.end_at) THEN RAISE EXCEPTION 'Appointment outside business hours or during closure' USING ERRCODE = 'check_violation';
END IF;
RETURN NEW;
END $$;
DROP TRIGGER IF EXISTS trg_validate_appointment ON work_order_appointments;
CREATE TRIGGER trg_validate_appointment BEFORE
INSERT
    OR
UPDATE ON work_order_appointments FOR EACH ROW EXECUTE FUNCTION bike_shop.validate_appointment();
...
SELECT l.id,
    ...
    '18:00'
FROM locations l
    CROSS JOIN generate_series(1, 6) d ON CONFLICT (location_id, dow) DO NOTHING;
CREATE OR REPLACE FUNCTION bike_shop.find_next_slot(
        _mechanic BIGINT,
        _location BIGINT,
        _duration_minutes INT,
        _from TIMESTAMPTZ DEFAULT now()
    ) RETURNS TABLE(start_at TIMESTAMPTZ, end_at TIMESTAMPTZ) LANGUAGE sql STABLE AS $$ WITH grid AS (
        SELECT gs AS slot_start,
            gs + make_interval(mins => _duration_minutes) AS slot_end
        FROM generate_series(
                _from,
                _from + interval '14 days',
                interval '15 minutes'
            ) gs
    ),
    allowed AS (
...
CREATE OR REPLACE FUNCTION bike_shop.find_next_slot_for_wo(
        ...
        _from TIMESTAMPTZ DEFAULT now()
    ) RETURNS TABLE(start_at TIMESTAMPTZ, end_at TIMESTAMPTZ) LANGUAGE sql STABLE AS $$ WITH inputs AS (
...
FROM bike_shop.find_next_slot(
        ...
        (
            SELECT minutes_needed
            FROM inputs
        ),
        _from
    );
$$;
CREATE OR REPLACE FUNCTION bike_shop.work_order_status_guard() RETURNS trigger LANGUAGE plpgsql AS $$
DECLARE ok BOOLEAN := FALSE;
BEGIN IF NEW.status = OLD.status THEN RETURN NEW;
END IF;
CASE
    OLD.status
    WHEN 'draft' THEN ok := NEW.status IN ('open', 'cancelled');
WHEN 'open' THEN ok := NEW.status IN (
    'in_progress',
    'awaiting_parts',
    'paused',
    'cancelled'
...
END CASE
;
IF NOT ok THEN RAISE EXCEPTION 'Illegal status transition: % -> %',
OLD.status,
NEW.status USING ERRCODE = 'check_violation';
END IF;
IF NEW.status IN ('completed', 'delivered')
AND OLD.closed_at IS NULL THEN NEW.closed_at := now();
END IF;
RETURN NEW;
END $$;
DROP TRIGGER IF EXISTS trg_wo_status_guard ON work_orders;
CREATE TRIGGER trg_wo_status_guard BEFORE
UPDATE OF status ON work_orders FOR EACH ROW EXECUTE FUNCTION bike_shop.work_order_status_guard();