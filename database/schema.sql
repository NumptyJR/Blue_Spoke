...
-- Sample data
INSERT INTO brands (name, is_component_brand) VALUES
    ('Trek', FALSE), ('Specialized', FALSE), ('Giant', FALSE),
    ('Cannondale', FALSE), ('Santa Cruz', FALSE), ('SRAM', TRUE),
    ('Shimano', TRUE), ('Fox', TRUE), ('RockShox', TRUE), ('Maxxis', TRUE)
ON CONFLICT DO NOTHING;

INSERT INTO categories (name) VALUES
    ('Bikes'), ('Drivetrain'), ('Brakes'), ('Suspension'),
    ('Wheels'), ('Tires'), ('Cockpit'), ('Bottom Bracket')
ON CONFLICT DO NOTHING;

INSERT INTO locations (name) VALUES ('Main Shop')
ON CONFLICT DO NOTHING;

INSERT INTO services_catalog (code, name, description, default_minutes, default_price) VALUES
    ('TUNE_BASIC','Basic Tune','General safety inspection and adjustment.',60,85.00),
    ('BB-SERVICE','Bottom Bracket Service','Remove, clean, inspect, re-grease/reinstall or replace bearings as needed.',45,60.00),
    ('BRAKE-BLEED','Brake Bleed (per brake)','Fluid replace & bleed; parts extra.',30,40.00),
    ('WHEEL-TRUE','Wheel True','Spoke tension & wheel alignment.',25,30.00),
    ('TUBELESS-SETUP','Tubeless Setup','Tape, valve, sealant for one wheel (parts extra).',20,25.00),
    ('LOWER-LEG','Suspension Lower-Leg Service','Fork lower-leg service; parts extra.',60,90.00)
ON CONFLICT DO NOTHING;

INSERT INTO users (email, full_name, role, is_active, password_hash) VALUES
    ('owner@bluespoke.test', 'Owner One', 'owner', TRUE, extensions.crypt('password', extensions.gen_salt('bf'))),
    ('manager@bluespoke.test', 'Manager Mary', 'manager', TRUE, extensions.crypt('password', extensions.gen_salt('bf'))),
    ('mechanic@bluespoke.test', 'Mechanic Max', 'mechanic', TRUE, extensions.crypt('password', extensions.gen_salt('bf')))
ON CONFLICT (email) DO NOTHING;

UPDATE users SET pin_code = '1001' WHERE email = 'owner@bluespoke.test'     AND pin_code IS NULL;
UPDATE users SET pin_code = '2002' WHERE email = 'manager@bluespoke.test'   AND pin_code IS NULL;
UPDATE users SET pin_code = '3003' WHERE email = 'mechanic@bluespoke.test'  AND pin_code IS NULL;
...
VALUES
    ...
    ('WH-002','29er Wheelset',2,5,FALSE,180,349,4),
    ('AC-003','Pro Tune Kit',3,7,FALSE,30,69,10)
...
INSERT INTO work_orders (status, customer_id, opened_by, assigned_to, notes, location_id)
SELECT 'open', c.id, owner.id, mech.id, 'Initial tune-up', loc.id
FROM customers c
JOIN users owner ON owner.role = 'owner'
LEFT JOIN LATERAL (SELECT id FROM users WHERE role = 'mechanic' LIMIT 1) mech(id) ON TRUE
JOIN locations loc ON loc.name = 'Main Shop'
WHERE c.id = 1
...
INSERT INTO work_order_appointments (work_order_id,start_at,end_at,assigned_to,location_id,status,notes)
SELECT w.id, now() + interval '1 day', now() + interval '1 day' + interval '2 hours', w.assigned_to, w.location_id, 'scheduled', 'Demo appointment'
FROM work_orders w
...
VALUES (
    'awesomepickle9@gmail.com',
    'Joshua schaff',
    'owner',
    TRUE,
    extensions.crypt('YourStrongPass!23', extensions.gen_salt('bf'))
)
ON CONFLICT (email) DO UPDATE
    SET is_active = EXCLUDED.is_active,
        password_hash = EXCLUDED.password_hash;

UPDATE users SET pin_code = '9009' WHERE email = 'awesomepickle9@gmail.com' AND pin_code IS NULL;
