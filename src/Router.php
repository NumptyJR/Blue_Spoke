...
final class Router
{
    ...
    public static function put(string $path, ...$handlers): void { self::add('PUT', $path, $handlers); }
    public static function patch(string $path, ...$handlers): void { self::add('PATCH', $path, $handlers); }
    public static function delete(string $path, ...$handlers): void { self::add('DELETE', $path, $handlers); }
    ...
    public static function group(array $middlewares, callable $define): void
    {
        ...
        // Ensure group middlewares (e.g., Auth) wrap route-specific ones (e.g., RequireRole)
        // so Auth executes first. Prepend group-level middlewares.
        foreach ($new as &$r) { $r['middlewares'] = array_merge($middlewares, $r['middlewares']); }
    ...
    {
        if (is_string($mw)) {
            return function() use ($mw, $next, $req) { (new $mw)->handle($req, $next); };
        }
        if (is_array($mw)) {
            [$cls, $arg] = $mw;
            return function() use ($cls, $arg, $next, $req) { (new $cls)->handle($req, $next, $arg); };
        }
        return function() use ($mw, $next, $req) { $mw($req, $next); };
    }
    ...
    private static function match(string $route, string $path, array &$outParams): bool
    {
        ...
        if (count($rSeg) !== count($pSeg)) return false;
